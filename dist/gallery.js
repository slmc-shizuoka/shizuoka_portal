...
const STORE = 'frames';
const FEATURED_KEY = 'shizuoka-portal-featured-frame';
const MAX_IMAGES = 3;
...
function setFeaturedId(id) {
  try {
    if (id) localStorage.setItem(FEATURED_KEY, id);
    else localStorage.removeItem(FEATURED_KEY);
  } catch { /* The selection remains optional when storage is unavailable. */ }
}

function updateFeaturedButton() {
  const button = $('#gallery-feature');
  const isFeatured = Boolean(selectedId && selectedId === featuredId());
  button.setAttribute('aria-pressed', String(isFeatured));
  button.textContent = isFeatured ? 'Featured on Portal' : 'Show on Portal';
}

function renderFeatured(frames) {
  const container = $('#hero-featured');
  const id = featuredId();
  const frame = id ? frames.find(item => item.id === id) : null;
  if (!frame || !(frame.blob instanceof Blob)) {
    container.hidden = true;
    $('#hero-featured-image').removeAttribute('src');
    if (id) setFeaturedId(null);
    return;
  }
  const url = URL.createObjectURL(frame.blob);
  activeUrls.push(url);
  const image = $('#hero-featured-image');
  image.src = url;
  image.alt = frame.caption || 'Galleryで選択した画像';
  $('#hero-featured-caption').textContent = frame.caption || 'Private Collection';
...
$('#gallery-feature').addEventListener('click', async () => {
  if (!selectedId) return;
  const willFeature = featuredId() !== selectedId;
  setFeaturedId(willFeature ? selectedId : null);
  await render();
  updateFeaturedButton();
  status.textContent = willFeature ? 'Portalに表示しました。' : 'Portalの表示を解除しました。';
});
...
$('#gallery-delete').addEventListener('click', async () => {
  ...
  try {
    if (featuredId() === selectedId) setFeaturedId(null);
